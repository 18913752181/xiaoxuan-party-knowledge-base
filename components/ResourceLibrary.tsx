            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              {frequentTopics.map(([name]) => (
                <button
                  key={name}
                  type="button"
                  onClick={() => router.push(`/library/all?topic=${encodeURIComponent(name)}`)}
        ...
        <section id="latest-materials" className="scroll-mt-28">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              ) : <Link href="/library/all" className="group inline-flex min-h-11 items-center rounded-xl border border-brand-line bg-white px-4 py-2.5 text-left transition-[border-color,background-color,transform,box-shadow] duration-150 hover:border-[#d9dde2] hover:bg-[#fff8f8] hover:shadow-[0_8px_18px_rgba(35,43,52,0.035)] active:scale-[0.98]"><span className="text-sm font-medium text-brand-ink transition-colors duration-150 group-hover:text-brand-red">全部专题</span></Link>}
              {libraryOnly ? (
                <>
          ...
          {!libraryOnly && !isLoading && filteredMaterials.length ? (
            <p className="mt-4 text-center text-xs text-neutral-400">
              首页仅展示最新 3 份，全部资料请前往
              <Link href="/library/all" className="ml-1 text-[#9a4650] hover:underline">全部资料</Link>
              。
            </p>
          ) : null}
          </>

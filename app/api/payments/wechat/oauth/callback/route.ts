...
export async function GET(request: Request) {
  ...
  const fail = (reason: string) =>
    NextResponse.redirect(`${siteUrl}/membership/payment?jsapi=error&reason=${encodeURIComponent(reason)}`, 302);
  ...
  const response = NextResponse.redirect(`${siteUrl}/membership/payment?jsapi=auto`, 302);
